...
LEVEL_GUIDE = {
    ...
    "advanced": (
        "학습자는 상급이다. **설명을 깊게** 한다 — 뉘앙스 차이, 어원, 비슷한 표현과의 비교, "
        "어떤 자리에서 쓰면 어색해지는지까지 짚어 준다.\n"
        "  난이도가 높다고 해서 **표현을 더 많이 뽑거나 쉬운 낱말까지 끌어오지 않는다.** "
        "고르는 기준은 초급·중급과 같고, 고른 것에 대한 설명만 깊어진다.\n"
        "  구문 해설은 3~5개까지 다룰 수 있다."
    ),
}
